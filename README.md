...
Projeto desenvolvido como requisito parcial para conclusão do módulo de desenvolvimento backend do curso de Desenvolvimento Web da Trybe. Neste projeto, são trabalhados conceitos de **SQL**, que incluem a criação de queries mais elaboradas, utilizando diferentes comandos em uma mesma query.

Neste projeto, são usados dois bancos de dados: o `hr` e um inédito chamado `w3schools`.
...
Neste projeto, são desenvolvidas as seguintes habilidades:
  ...
  * Criação de condicionais no **SQL** usando **IF** e **CASE**

  * Manipulação de _strings_ no **SQL**

  * Uso de diversas funções matemáticas do **MySQL**

  * Extração de informações específicas sobre datas de uma tabela

  * Utilização das funções de agregação **AVG**, **MIN**, **MAX**, **SUM** e **COUNT**

  * Exibição e filtro de dados de forma agrupada com **GROUP BY** e **HAVING**

  * Utilização de **INNER JOIN**, **LEFT JOIN**, **RIGHT JOIN** para combinar dados de duas ou mais tabelas

  * Utilização de **SELF JOIN** para fazer join de uma tabela com ela própria

  * Utilização de SUBQUERIES

  * Criação de queries mais eficientes através do EXISTS

  * Montagem de blocos de código **SQL** reutilizáveis com **STORED PROCEDURES** e **STORED FUNCTIONS**
